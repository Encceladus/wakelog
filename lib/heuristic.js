...
// Last resort for programs no rule knows about.
// It does not claim certainty; it marks a command as suspect, for a human to look at.
...
const VERBS = {
  ...
  overwrite: 3, rollback: 2, downgrade: 2, terminate: 3,
};
...
// programs where a destructive verb is ordinary, reversible work
...
// splits a token into parts, but only when it does not look like a path or filename
...
// --format, --reset-cache and --clean are ordinary flags; only these stems count in flags
const FLAG_STEMS = new Set([
  'delete', 'purge', 'prune', 'destroy', 'remove', 'expunge', 'drop',
  'wipe', 'erase', 'uninstall', 'revoke', 'flush', 'truncate', 'shred', 'yank', 'vacuum',
]);
...
function scan(ctx) {
  ...
  for (let i = 1; i < ctx.argv.length; i++) {
    ...
    if (tok.startsWith('-')) {
      let w = FLAGS[tok] !== undefined ? FLAGS[tok] : null;
      if (w === null && FLAG_STEMS.has(bare)) w = VERBS[bare];
      if (w === null) {
        const stem = [...FLAG_STEMS].find((v) => bare.startsWith(v + '-'));
  ...
  // the program name itself is sometimes the verb: shutdown, reboot, lvremove, userdel
