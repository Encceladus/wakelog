...
function classifySimple(cmd) {
  ...
  if (name === 'rm') {
    ...
    { const bad = pos.find(looksOutsideProject); if (bad) return hit(L1, 'rm-outside', 'rm-outside', { path: bad }); }
  ...
  if (name === 'mv' || name === 'cp') {
    { const bad = pos.find(looksOutsideProject); if (bad) return hit(L1, 'fs-outside', 'fs-outside', { path: bad }); }
...
function classifyRedirects(cmd) {
  for (const r of cmd.redirects) {
    ...
    if (looksOutsideProject(r.target)) {
      return { level: L1, reason: 'redirect-outside', tag: 'redirect-outside', params: { path: r.target } };
    }
    if (SECRETISH.test(r.target)) {
      return { level: L1, reason: 'redirect-secret', tag: 'redirect-secret', params: { path: r.target } };
    }
  }
  return null;
}
